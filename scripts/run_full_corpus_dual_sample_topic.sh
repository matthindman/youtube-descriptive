...
NOTEBOOK_PATH="${WORKSPACE_DIR}/13_full_corpus_dual_sample_topic_model_databricks"
...
JOB_JSON="$(mktemp "${TMPDIR:-/tmp}/full_corpus_dual_sample_topic.XXXXXX.json")"
cleanup() {
  rm -f "${JOB_JSON}"
}
trap cleanup EXIT
...
"${DATABRICKS_CMD[@]}" workspace import "${NOTEBOOK_PATH}" \
  --file "${ROOT_DIR}/youtube_descriptive/src/13_full_corpus_dual_sample_topic_model_databricks.py" \
  --format SOURCE --language PYTHON --overwrite
...
python3 "${ROOT_DIR}/scripts/build_full_corpus_dual_sample_topic_job.py" \
  --config "${LOCAL_CONFIG}" \
  --output "${JOB_JSON}" \
...
echo "Submitting model-completed topic job on existing cluster ${CLUSTER_ID}."
