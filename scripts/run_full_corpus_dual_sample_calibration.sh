...
NOTEBOOK_PATH="${WORKSPACE_DIR}/17_full_corpus_dual_sample_topic_calibration_databricks"
...
JOB_JSON="$(mktemp "${TMPDIR:-/tmp}/full_corpus_dual_sample_calibration.XXXXXX.json")"
cleanup() {
  rm -f "${JOB_JSON}"
}
trap cleanup EXIT
...
"${DATABRICKS_CMD[@]}" workspace import "${NOTEBOOK_PATH}" \
  --file "${ROOT_DIR}/youtube_descriptive/src/17_full_corpus_dual_sample_topic_calibration_databricks.py" \
  --format SOURCE --language PYTHON --overwrite
"${DATABRICKS_CMD[@]}" fs mkdir "${DBFS_DIR}"
"${DATABRICKS_CMD[@]}" fs cp "${LOCAL_CONFIG}" "${DBFS_CONFIG}" --overwrite

python3 "${ROOT_DIR}/scripts/build_full_corpus_dual_sample_calibration_job.py" \
  --config "${LOCAL_CONFIG}" \
  --output "${JOB_JSON}" \
...
echo "Submitting human-validation topic calibration on existing cluster ${CLUSTER_ID}."
