#!/usr/bin/env python3
"""Build the model-completed topic robustness job payload."""
...
def build_payload(args: argparse.Namespace) -> dict[str, object]:
    ...
    for index, stage in enumerate(STAGES):
        ...
        if index:
            task["depends_on"] = [{"task_key": f"topic_{STAGES[index - 1]}"}]
        tasks.append(task)
    return {"run_name": f"{config['design_version']}_topic_model", "tasks": tasks}
