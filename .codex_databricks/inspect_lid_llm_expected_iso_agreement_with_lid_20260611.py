...
sample_channels = selected_llm_raw.select("channel_id").distinct()

llm_votes = (
    selected_llm_raw.where(F.col("is_valid_panel_vote") == F.lit(True))
    .select(
        "channel_id",
        "provider",
        "model",
...
model_counts = (
    ...
    .agg(F.countDistinct("channel_id").alias("n_valid_panel_votes"))
...
pairwise = (
    ...
    .agg(
        ...
        F.sum(
            F.when(
                (F.col("a.base_iso") != F.col("b.base_iso"))
                & (F.col("a.normalized_base_iso") == F.col("b.normalized_base_iso")),
                1,
            ).otherwise(0)
        ).alias("n_raw_disagreements_resolved_by_normalization"),
    )
    ...
    .withColumn("normalized_minus_raw_pp", F.round((F.col("normalized_base_iso_agreement_rate") - F.col("raw_base_iso_agreement_rate")) * F.lit(100), 2))
...
overall = (
    pairwise.agg(
        ...
        F.sum("n_raw_base_iso_agree").alias("raw_base_iso_agree"),
        F.sum("n_normalized_base_iso_agree").alias("normalized_base_iso_agree"),
        F.sum("n_raw_disagreements_resolved_by_normalization").alias("raw_disagreements_resolved_by_normalization"),
    )
    .withColumn("raw_base_iso_agreement_rate", F.round(F.col("raw_base_iso_agree") / F.col("pair_channel_comparisons"), 4))
...
result = {
    "source": {
        ...
        "lid_channel_model_aggregation_table": LID_CHANNEL_MODEL_AGGREGATION_TABLE,
        "normalization": {
